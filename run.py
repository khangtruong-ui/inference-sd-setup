...
def load_model():
    pipeline, params = FlaxStableDiffusionPipeline.from_pretrained(
        os.environ['STABLE_DIFFUSION_DIRECTORY']
    )
...
def generate_from_prompts(pipeline, params, prompts: list[str]):
    ...
    images = pipeline(prompt_ids, params, subkey, num_inference_steps=1000, guidance_scale=np.array([7.5]), height=256, width=256)
...
def load_prompts():
    ...
    iterables = ds.iter(batch_size=128, drop_last_batch=True)
